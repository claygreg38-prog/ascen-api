...
export default function BottomNav({ features }) {
  const loc = useLocation();
  const nav = useNavigate();

  const tabs = [
    { path: '/app/', label: 'Home', icon: ICONS.home, always: true },
    { path: '/app/gallery', label: 'Gallery', icon: ICONS.gallery, always: true },
  ];

  // Family tab: only if features.family_hub is true
  if (features?.family_hub) {
    tabs.push({ path: '/app/family', label: 'Family', icon: ICONS.family });
  }
  // Partner tab: only if features.partner_hub is true
  if (features?.partner_hub) {
    tabs.push({ path: '/app/family', label: 'Partner', icon: ICONS.family });
  }

  tabs.push({ path: '/app/profile', label: 'Profile', icon: ICONS.profile, always: true });
  ...
  return (
    <nav style={{
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.path + t.label} onClick={() => nav(t.path)} style={{
            background: 'none', border: 'none', display: 'flex', flexDirection: 'column',
            alignItems: 'center', gap: '2px', padding: '8px 16px', cursor: 'pointer',
            color: active ? '#4fd1c5' : 'rgba(160,196,232,0.4)', minWidth: '64px'
          }}>
